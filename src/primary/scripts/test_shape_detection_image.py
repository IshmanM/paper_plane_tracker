...
def createStageGridImage(stages: list[tuple[str, np.ndarray]], columns: int) -> np.ndarray:
    ...
    for stage_index, (stage_name, stage_image) in enumerate(display_stages):
        ...
        cv2.rectangle(grid_image, (tile_x, tile_y), (tile_x + tile_width - 1, tile_y + tile_height - 1), (70, 70, 70), 1)
...
def showDetectionStages(stages: list[tuple[str, np.ndarray]], columns: int) -> None:
    ...
    def zoomAt(mouse_x: int, mouse_y: int, zoom_factor: float) -> None:
        viewport_height = getViewportHeight()
        mouse_x, mouse_y = min(max(mouse_x, 0), state["window_width"]), min(max(mouse_y, 0), viewport_height)
        ...
        canvas_x, canvas_y = (mouse_x - state["offset_x"])/old_zoom, (mouse_y - state["offset_y"])/old_zoom

        state["zoom"] = new_zoom
        state["offset_x"], state["offset_y"] = mouse_x - canvas_x*new_zoom, mouse_y - canvas_y*new_zoom
        clampView()
    ...
    while cv2.getWindowProperty(WINDOW_NAME, cv2.WND_PROP_VISIBLE) >= 1:
        ...
        window_width, window_height = max(1, window_width), max(STATUS_BAR_HEIGHT + 1, window_height)
        ...
        viewport_height = getViewportHeight()
        transform = np.float32([[state["zoom"], 0.0, state["offset_x"]], [0.0, state["zoom"], state["offset_y"]]])
        ...
        cv2.putText(display_image, f"Wheel or +/-: zoom    Drag: pan    R/double-click: reset    Q/Esc: quit    Zoom: {state['zoom']:.2f}x",
...
def main() -> None:
    ...
    if detection is None:
        ...
        drawModelOrigin(measurement_frame, measurement)
        ...
        debug.addStage("World-space measurement + model origin", measurement_frame)
